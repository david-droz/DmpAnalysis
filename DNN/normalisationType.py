...
def _normalise(arr,atOne):
	...
	if not atOne:
		...
		for i in range(arr.shape[1]):
			if np.all(arr[:,i] > 0) :
				arr[:,i] = (arr[:,i] - np.mean(arr[:,i]) + 1.) / np.std(arr[:,i])
			else:
				arr[:,i] = (arr[:,i] - np.mean(arr[:,i])) / np.std(arr[:,i])	
	return arr
...
def getParticleSet(fname,normtype):
	arr = np.load(fname)
	X = arr[:,0:-2]				### Last two columns are timestamp and particle id
	Y = arr[:,-1]
	X = _normalise(X,normtype)
...
def run(normtype):
	...
	train_e = getParticleSet('/home/drozd/analysis/data_train_elecs.npy',normtype)
	train_p = getParticleSet('/home/drozd/analysis/data_train_prots.npy',normtype)
	...
	val_e = np.concatenate((getParticleSet('/home/drozd/analysis/fraction1/data_validate_elecs_1.npy',normtype) , getParticleSet('/home/drozd/analysis/fraction1/data_test_elecs_1.npy',normtype) ))
	val_p = np.concatenate((getParticleSet('/home/drozd/analysis/fraction1/data_validate_prots_1.npy',normtype) , getParticleSet('/home/drozd/analysis/fraction1/data_test_prots_1.npy',normtype) ))
